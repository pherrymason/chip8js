...
p._executeOpcode = function(opcode){
	...
	switch(opcode & 0xF000){
		...
		case 0x0000:
					...
					switch( opcode )
					{
						...
						case 0x00E0:
									...
									this._incrementPC();
									this.options.debug && (description = 'Clears screen');
									break;

						// Returns from subroutine
						case 0x00EE:
									this.pc = this.stack.pop();
									this.options.debug && (description = 'Returns from subroutine');
									break;

						default:	// 0x0NNN
									this._incrementPC();
									this.options.debug && (description = 'Calls RCA 1802 program at 0x' + NNN.toString(16).toUpperCase() );
					}
					break;
		...
		case 0x1000:
					this.pc = NNN;
					this.options.debug && (description = 'Jumps to '+NNN.toString(16));
					break;
		...
		case 0x2000:
					...
					this.pc = NNN;
					this.options.debug && (description = 'Call subroutine at '+NNN.toString(16));
					break;
		...
		case 0x3000:
					if( this.V[ X ] == NN )
					{
						this._incrementPC();
						this._incrementPC();
						this.options.debug && (description = 'Skips next instruction cause V['+X+']==0x'+NN.toString(16));
					}
					else
					{
						this._incrementPC();
		...
		case 0x4000:
					...
					{
						this._incrementPC();
						this.options.debug && (description = 'Does not skip next instruction cause V['+X+']==0x'+NN.toString(16));
					}
					break;
		...
		case 0x5000:
					if( this.V[ X ]==this.V[ Y ] )
					{
						this._incrementPC();
						this._incrementPC();
						this.options.debug && (description = 'Skips the next instruction case V['+X+']==V['+Y+']');
					}
					else
					{
						this._incrementPC();
		...
		case 0x6000:
					this.V[ X ] = NN;
					this._incrementPC();
					this.options.debug && (description = 'Sets V[' + X + '] to ' + NN.toString(16));
					break;
		...
		case 0x7000:
					this.V[ X ]+= NN;
					this._incrementPC();
					this.options.debug && (description = 'Adds '+NN.toString(16)+' to V[' + X + ']');
					break;
		...
		case 0x8000:
					switch( opcode&0x000F )
					{
						...
						case 0:
								this.V[X] = this.V[Y];
								this.options.debug && (description = 'Sets V['+X+'] to V['+Y+']');
								break;
	
						// 8XY1	Sets VX to "VX OR VY".
						case 1:
								this.V[X]|= this.V[Y];
								this.options.debug && (description = 'Sets V['+X+'] to "V['+X+'] OR V['+Y+']"');
								break;
		
						// 8XY2	Sets VX to "VX AND VY".
						case 2:
								this.V[X]&= this.V[Y];
								this.options.debug && (description = 'Sets V['+X+'] to "V['+X+'] AND V['+Y+']"');
								break;

						// 8XY3	Sets VX to "VX xor VY".
						case 3:
								this.V[X]^= this.V[Y];
								this.options.debug && (description = 'Sets V['+X+'] to "V['+X+'] XOR V['+Y+']"');
								break;
						...
						case 4:
								...
								this.V[Y]+= this.V[X];
								this.options.debug && (description = 'Add V['+X+'] to V['+Y+']. VF carries');
								break;
						...
						case 5:
								...
								this.V[X]-= this.V[Y];
								this.options.debug && (description = 'Substracts V['+X+'] - V['+Y+']. VF borrows');
								break;
						...
						case 6:
								...
								this.V[X] >>= 1;
								this.options.debug && (description = 'Shifts V['+X+']>>1. VF.');
								break;
						...
						case 7:
								...
								this.options.debug && (description = 'Sets V['+X+'] = V['+Y+']-V['+X+']. VF borrows.');
								break;
		...
		case 0x9000:
					if( this.V[ X ] != this.V[ Y ] )
					{
						this._incrementPC();
						this._incrementPC();
						this.options.debug && (description = 'Skips next instruction cause V['+X+'] != V['+Y+']');
					}
					else
					{
						this._incrementPC();
		...
		case 0xA000:
					this.I = NNN;
					this._incrementPC();
					this.options.debug && (description = 'Sets I to the address '+NNN.toString(16));
					break;
		...
		case 0xB000:
					...
					this.options.debug && (description = 'Jumps to address '+NNN.toString(16)+' plus V[0]');
					break;
		...
		case 0xC000:
					...
					this._incrementPC();
					this.options.debug && (description = 'Sets V['+X+'] to a random number ('+this.V[X]+')');
					break;
		...
		case 0xE000:
					...
					this._incrementPC();
					this.options.debug && (description = '????');
					break;
		...
		case 0xF000:
					switch (opcode&0x00FF){
						...
						case 0x07:
									...
									this.options.debug && (description = 'sets V[X] the value of timer delay.');
									break;

						// FX0A	A key press is awaited, and then stored in VX.
						case 0x0A:
									this.options.debug && (description = 'Waits for keypress.');
									var keypress = false;
									for (var i=0; i<16; i++){
						...
						case 0x15:
									...
									this.options.debug && (description = 'Sets the delay timer to V[X].');
									break;

						// FX18	Sets the sound timer to VX.
						case 0x18:
									this.timerSound = this.V[X];
									this.options.debug && (description = 'Sets the sound timer to V[X].');
									break;
						...
						case 0x1E:
									...
									this.options.debug && (description = 'Adds V[X] to I.');
									break;

						// FX29	Sets I to the location of the sprite for the character in VX. Characters 0-F (in hexadecimal) are represented by a 4x5 font.
						case 0x29:
									this.I = this.V[X] * 0x5;	// ????
									this.options.debug && (description = 'Sets I to the location of the sprite.');
									break;
						...
						case 0x33:
									...
									this.options.debug && (description = 'Stores binary coded decimal');
									break;
						...
						case 0x55:
									...
									this.I+= X+1;
									this.options.debug && (description = 'Fills V[0] to V['+X+'] in memory from '+this.I);
									break;
						...
						case 0x65:
									...
									this.I+= X+1;
									this.options.debug && (description = 'Fills V[0] to V['+X+'] with memory from '+this.I);
									break;
					}
					this._incrementPC();
					break;
